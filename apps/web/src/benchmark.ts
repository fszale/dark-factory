...
world.scene.onAfterRenderObservable.add(() => {
  ...
  if (document.visibilityState === "visible") {
    ...
    if (frameTimes.length > 7200) frameTimes.shift();
  }
  lastFrame = now;
});
...
world.onStats = (fps, meshes) => {
  ...
  const p95ms = frames[Math.floor((frames.length - 1) * 0.95)] || 0;
  const p99ms = frames[Math.floor((frames.length - 1) * 0.99)] || 0;
  const p5 = sorted[Math.floor((sorted.length - 1) * 0.05)];
  document.querySelector("#fps")!.textContent = `${fps.toFixed(1)} FPS`;
  document.querySelector("#stats")!.textContent =
    `Window ${windowNumber} · ${((performance.now() - started) / 1000).toFixed(0)} seconds · ${samples.length} samples · ${meshes} meshes · mean ${(samples.reduce((a, b) => a + b, 0) / samples.length).toFixed(1)} FPS · minimum ${Math.min(...samples).toFixed(1)} FPS · 5th percentile ${p5.toFixed(1)} FPS · ${innerWidth} × ${innerHeight} · frame p95 ${p95ms.toFixed(1)} ms / p99 ${p99ms.toFixed(1)} ms (${frames.length} foreground frames)`;
